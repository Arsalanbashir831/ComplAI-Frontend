...
export function ResolverChatBubble({
    ...
    const isLoading = (message.content === '' || message.content === 'loading') && !message.done;

    const hasReasoning = message.reasoning && message.reasoning.trim().length > 0;
    const shouldShowReasoning = hasReasoning && (message.content === '' || message.content === 'loading');
    ...
    useEffect(() => {
        if (!isBot || !message.content || message.done || message.content === 'loading') {
    ...
    useEffect(() => {
        ...
        if (currentReasoning.length > previousReasoning.length) {
            ...
            reasoningTimeoutRef.current = setTimeout(() => setIsReasoningStreaming(false), 2000);
        }
                    ...
                    <div className="flex-1 flex flex-col gap-2 min-w-0">
                            ...
                            >
                                <div className="w-full flex items-center justify-between p-3 bg-gray-50 relative overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-2 relative z-10">
                                        ...
                                        {isReasoningStreaming && (
                                            <span className="text-xs text-blue-600 animate-pulse">Thinking...</span>
                                        )}
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        {isError ? (
                            <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg p-4 text-sm">
                                ...
                                {onRetry && (
                                    <Button onClick={onRetry} variant="outline" size="sm" className="mt-2 border-red-200 text-red-700 hover:bg-red-100">
                                        Retry
                                    </Button>
                                )}
                            </div>
