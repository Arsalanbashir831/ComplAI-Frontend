...
export default function ResolverResponsePage() {
    const { id } = useParams();
    const { sendMessage, useComplaintDetails, useKeyPoints, useMessagesList } = useResolver();

    // Queries
    const { data: details, isLoading: isLoadingDetails } = useComplaintDetails(id as string);
    const { data: messagesData, isLoading: isLoadingMessages } = useMessagesList(id as string);
    const { data: keyPointsData, isLoading: isLoadingKeyPoints } = useKeyPoints(id as string);
    ...
    useEffect(() => {
        if (messagesData?.results && chatMessages.length === 0) {
    ...
    }, [messagesData, id, chatMessages.length]);

    // Auto-trigger first analysis if no messages exist
    useEffect(() => {
        if (!isLoadingMessages && !isLoadingDetails && messagesData && details && !hasTriggeredAuto) {
            // Check if there are no user/assistant messages
            const existingMessages = messagesData.results.filter(m => m.role !== 'system');

            if (existingMessages.length === 0) {
                setHasTriggeredAuto(true);
                const initialPrompt = details.context?.system_prompt || 'Analyze this complaint and provide a professional response.';
    ...
    }, [isLoadingMessages, isLoadingDetails, messagesData, details, hasTriggeredAuto]);
        ...
        return (
            <div className="flex h-screen w-full items-center justify-center bg-white">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-[#04338B] font-medium">Loading complaint details...</p>
                </div>
            </div>
        );
    }
